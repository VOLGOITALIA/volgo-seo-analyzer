...
class SEOAnalyzer:
    def __init__(self):
        ...
        # URL (path) da escludere dall'analisi
        self.excluded_paths = [
            '/privacy-policy',
            '/cookie-policy',
        ...
        # Sessione opzionale per rendering JS (se requests_html è installato)
    ...
    def scan_website_pages(self, base_url: str, sitemap_urls: List[str]) -> List[Dict]:
        ...
        if sitemap_urls:
            ...
            for u in sitemap_page_urls:
                if not self._is_excluded_url(u, base_url):
                    urls_to_scan.add(u)
        ...
        # Aggiungi sempre l'URL base (se non è una pagina esclusa)
        if not self._is_excluded_url(base_url, base_url):
            urls_to_scan.add(base_url)

        # Filtra di nuovo per sicurezza e limita il numero di pagine
        urls_to_scan = [u for u in urls_to_scan if not self._is_excluded_url(u, base_url)]
    ...
    def _is_excluded_url(self, url: str, base_url: str) -> bool:
        """Ritorna True se l'URL va escluso dall'analisi (privacy, cookie, terms, ecc.)"""
        try:
            parsed = urlparse(url)
            base_parsed = urlparse(base_url)

            # domini diversi? non si applica la regola di esclusione
    ...
    def _analyze_page(self, url: str) -> Dict:
        ...
        try:
            ...
            # Se ancora vuoto o quasi, prova a eseguire il JavaScript (se disponibile)
            if (not text_content or len(text_content.strip()) < 50) and self.js_session is not None:
                try:
                    js_response = self.js_session.get(url, timeout=self.timeout)
                    js_response.html.render(timeout=self.timeout, sleep=1)
                    rendered_html = js_response.html.html or ""
                    if rendered_html:
                        # Aggiorna il soup principale con l'HTML renderizzato
                        soup = BeautifulSoup(rendered_html, 'html.parser')

                        # Riprova con trafilatura sul contenuto renderizzato
                        text_content = trafilatura.extract(rendered_html) or ""
                        if not text_content or len(text_content.strip()) < 50:
                            soup_copy = BeautifulSoup(rendered_html, 'html.parser')
                            for script in soup_copy(["script", "style", "nav", "header", "footer"]):
                                script.decompose()
                            text_content = soup_copy.get_text()
                            lines = (line.strip() for line in text_content.splitlines())
                    ...
                    # Se il rendering JS fallisce, teniamo quello che abbiamo
                    pass
    ...
    def _extract_headings(self, soup: BeautifulSoup) -> Dict:
        ...
        if not headings['h1']:
            ...
            for elem in elementor_h1:
                parent = elem.parent
                if parent and (
                    'h1' in str(parent.get('class', [])).lower()
                    or elem.get('data-level') == '1'
                ):
        ...
        if len(headings['h2']) < 2:
            ...
            for elem in elementor_h2:
                parent = elem.parent
                if parent and (
                    'h2' in str(parent.get('class', [])).lower()
                    or elem.get('data-level') == '2'
                ):
                    text = elem.get_text(strip=True)
                    if text and 3 < len(text) < 200 and text not in headings['h2']:
        ...
        if len(headings['h3']) < 3:
            ...
            for elem in elementor_h3:
                parent = elem.parent
                if parent and (
                    'h3' in str(parent.get('class', [])).lower()
                    or elem.get('data-level') == '3'
                ):
                    text = elem.get_text(strip=True)
                    if text and 3 < len(text) < 200:
    ...
    def _is_valid_content_image(self, src: str, alt: str) -> bool:
        ...
        if any(ext in src_lower for ext in ['.svg', '.gif']) and any(
            word in src_lower for word in ['icon', 'logo', 'bullet']
        ):
            return False
    ...
    def _analyze_titles(self, pages_data: List[Dict]) -> Dict:
        ...
        if good_titles < len(pages_data) * 0.8:
            ...
            recommendations.append(
                "Assicurati che ogni pagina abbia un titolo unico e descrittivo"
            )
    ...
    def _analyze_headings(self, pages_data: List[Dict]) -> Dict:
        """Analizza la struttura completa degli heading (H1, H2, H3)"""
        issues = []
        successes = []
        recommendations = []
    ...
    def _analyze_images_alt(self, pages_data: List[Dict]) -> Dict:
        ...
        if total_images > 0:
            if images_with_alt < total_images * 0.9:
                recommendations.append(
                    "Aggiungi attributi alt descrittivi a tutte le immagini"
                )
    ...
    def _analyze_content_length(self, pages_data: List[Dict]) -> Dict:
        ...
        if good_content < len(pages_data) * 0.8:
            recommendations.append(
                "Aumenta la lunghezza del contenuto (minimo 300 caratteri)"
            )
    ...
    def _analyze_keyword_density(self, pages_data: List[Dict]) -> Dict:
        """Analisi base della densità delle parole chiave"""
        return {
            'score': 75,
            'issues': [],
    ...
    def _analyze_mobile_friendly(self, pages_data: List[Dict]) -> Dict:
        """Analizza la compatibilità mobile"""
        issues = []
        recommendations = []
        successes = []
                ...
                try:
                    ...
                    if width_match:
                        ...
                        if width >= 768:
                            mobile_friendly_pages += 1
                            successes.append(
                                f"Viewport personalizzato per CMS con versione mobile "
                                f"dedicata (width={width}): {url}"
                            )
                        else:
                            mobile_friendly_pages += 1
